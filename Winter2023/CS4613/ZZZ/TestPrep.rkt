...
(module question2 plait #:untyped
  ...
  (define (typecheck [ae : AE]) : Type
    (type-case AE ae
      ...
      [(Add l r)
       (cond
         [(and (NumT? (typecheck l)) (NumT? (typecheck r))) (NumT)]
         [(and (BoolT? (typecheck l)) (BoolT? (typecheck r))) (BoolT)]
  ...
  (module free-vars plait
    ...
    (define (free-vars expr env)
      (type-case FLANG expr
        ...
        [(Let1 bound-id named-expr bound-body) (free-vars bound-body (extend bound-id env))]
...
(module free-vars plait
    ...
    (define (free-vars expr env)
      (type-case FLANG expr
        [(Num n) empty-env]
        [(Add l r) (union (free-vars l env) (free-vars r env))]
        [(Let1 bound-id named-expr bound-body) (free-vars named-expr (if (bound bound-id env) empty-env (extend bound-id empty-env)))]
        [(Id name) (if (bound name env) empty-env (extend name empty-env))]
        [(Fun bound-id bound-body) (free-vars bound-body env)]
        [(Call fun-expr arg-expr) (free-vars fun-expr empty-env)]))
...
(define-type VAL
  ...
  [LamV (id : Symbol) (body : FLANG) (env : ENV)])
...
(define (lookup name env)
  ...
  (type-case FLANG expr
    [(Num n) (NumV n)]
    [(Add l r)
     (Num (+ (Num-val (interp l env)) (Num-val (interp r env))))]
    [(Let1 bound-id named-expr bound-body)
...
(require (only-in 'free-vars))
(require (only-in 'question4))

(require (only-in 'lambda-sum))