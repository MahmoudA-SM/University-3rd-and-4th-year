...
//      <w_n1 w_n2 w_n3 ... w_nd>
    ...
    int delta1000_int = atoi(Param::read(argc, argv, "-del", ""));
    double delta=((double)delta1000_int)/1000.0;
...
//            int generated_r_cnt= utk_efficient_anti(PointSet, dim, w, rtree, m, k, utk_option_ret,utk_cones_ret);
    ...
    if (strcmp(methodName, "AORU_OA3") == 0) // ORU efficient
    {
        ...
        for (int wi = 0; wi < w_num; wi++)
        {
            ...
            // the code commented below just for anti data
//            int generated_r_cnt= utk_efficient_anti(PointSet, dim, w, rtree, m, k, utk_option_ret,utk_cones_ret);
            int generated_r_cnt= utk_efficient3(PointSet, dim, w, rtree, m, k, utk_option_ret, utk_cones_ret, delta);
            cout<<"ret size: "<<utk_option_ret.size()<<"\n";
            for (int i = 0; i < avg_radius.size(); ++i) {
                if(i<utk_option_ret.size()){
                    avg_radius[i]+=utk_option_ret[i].second;
    ...
    if (strcmp(methodName, "DORU") == 0) // ORU efficient
    {
        ...
        for (int wi = 0; wi < w_num; wi++)
        {
            ...
            for (int i = 0; i < m; ++i) {
                auto id_dist=obj.get_next();
                // 第几个: 产品的id, 产品的距离
//                cout<<"ORD:"<<i+1<<":"<<id_dist.first<<","<<id_dist.second<<endl;
            }

            float rho = obj.interval.back().second;
            cout << "The ORD inflection radius is: " << rho<< "#" << obj.interval.size() << endl;
            unordered_map<int, double> options=doru_p1(obj.drill_position, PointSet, k, w);
            vector<pair<int, double>> final_options=drill_by_grid(w, PointSet, dim, k, m, delta, options);
