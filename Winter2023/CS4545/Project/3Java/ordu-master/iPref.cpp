...
bool add_box(multimap<double, box*> &heap, vector<int> &tmp, vector<float> &base,
    ...
    vector<double> minus50f(dim_m1, -0.5);
    vector<double> tmp2 = minus50f + tmp;
...
//        if(0>=i-1) {
//            if(i+1>=dim_m1){
//                ret_cnt+=add_box(heap, tmp, base, dim_m1, e);
//            }
//            for (int l = i + 1; l<dim_m1; ++l) {
//                // from 0 to n
//                for (int m = 0; m < n; ++m) {
//                    tmp[l] = m;
//                    ret_cnt+=add_box(heap, tmp, base, dim_m1, e);
...
vector<pair<int, double>> drill_by_grid(vector<float>&w,float** P, int dim, int k, int m, double delta, unordered_map<int, double> &p1_options){
    // for doru
    ...
    auto begin=chrono::steady_clock::now();
    vector<pair<int, double>> p1_optionv(p1_options.begin(), p1_options.end());
    sort(p1_optionv.begin(), p1_optionv.end(), [](auto &a, auto &b){return a.second>b.second;});
...
//    unordered_map<int, double> options;
    ...
    cout<<"debug: delta/(dim-1), "<<delta/(dim-1)<<endl;
    for(auto &&ee:e){
        cout<<ee<<"\n";
    }
    int n=1;
...
//    vector<vector<box>> heap_ele_ref_maintain
    int ret=0;
    unordered_set<int> ret_options;
    ...
    while (ret_options.size() < m) {
        ...
        while(!heap.empty() && heap.begin()->first<n*delta/(dim-1)){
            // be careful here, we should return the options with minimum distance
            while(!p1_optionv.empty() && p1_optionv.back().second < heap.begin()->first){// add phase 1 options
                auto iter=ret_options.find(p1_optionv.back().first);
                if(iter==ret_options.end()) {// new option
                    ret_options.insert(p1_optionv.back().first);
                    ret_idDis.emplace_back(p1_optionv.back());
                }
                p1_optionv.pop_back();
            }
            if(ret_options.size() >= m){
                break;
            }
            box *top=heap.begin()->second;
            vector<double> center=top->center();
            vector<int> topk=topk_single(k, P, center, 0, rtree, ramTree);
            for(int opt:topk){
                auto iter=ret_options.find(opt);
                if(iter==ret_options.end()) {// new option
                    ret_idDis.emplace_back(opt, heap.begin()->first);
                    ret_options.insert(opt);
                    if(ret_options.size() >= m){
                        break;
                    }
//                    cout << "radius: " << heap.begin()->first << "\n";
//                    if(true){
//                        cout << options.size() << ": " << heap.begin()->first << "# " << opt << "\n";
//                    }
//                    auto now = chrono::steady_clock::now();
//                    chrono::duration<double> elapsed_seconds= now-begin;
//                    cout << "time: " << options.size() << ", " << elapsed_seconds.count()<<"\n";
    ...
    for (auto i=p1_optionv.rbegin(); i!=p1_optionv.rend();++i) {
        if(ret_options.size()>=m){
            break;
        }
        auto iter=ret_options.find(i->first);
        if(iter==ret_options.end()){
            ret_options.insert(i->first);
            ret_idDis.emplace_back(*i);
        }
    }
...
unordered_map<int, double> doru_p1(vector<vector<double>> &points, float** P, int k, vector<float> &w){
    auto begin = chrono::steady_clock::now();
    unordered_map<int, double> options;
    for(auto &center: points) {
        ...
        for (int opt:topk) {
            auto iter = options.find(opt);
            if (iter == options.end()) {// new option
                double distd=dist(w, center);
                options.emplace(opt, distd);
//                cout << "dour p1 radius: " << distd << "\n";
            }
        }
    }
    auto now = chrono::steady_clock::now();
        ...
        if(popped.second->top_what==k){ // a region that don't need to be partitioned
            ...
            if(delta>0) {
                ...
                if ( md< delta) {
                    // center of mmb
//                    cout<<"cut\n";
                    vector<int> topk=topk_single(k, PG, center_mbb, 0, rtree, ramTree);
                    for(int opt:topk){
                        auto iter=options.find(opt);
                        if(iter==options.end()) {// new option
                            options.insert(opt);
...
//            cout<<"result size: "<<next_topi.size()<<"\n";
            ...
            // TODO some regions was espcaced since accuracy infeasible
//            if(topip1.empty()){
//                vector<int> topk=topk_single(k, PG, center_mbb, 0, rtree, ramTree);
//                for(int opt:topk){
//                    auto iter=options.find(opt);
...
//    vector<vector<double>> begin_region=points_to_halfspace(tmp);
    ...
    int ret=topRegions_efficient3(begin_region, ch_obj, id_radius,  PointSet, dim, X,
                                  k,  w, top1_calculated, utk_option_ret, utk_cones_ret_, top1_region, delta);
    delete(root);
    delete(top1r);
